...
<head>
    <meta charset="UTF-8">
    <title>選擇日期 - <%= clinic.getName() %></title>
    <style>
        ...
        input[type="date"] { padding:12px; font-size:18px; width:100%; margin:20px 0; }
        button { padding:15px 30px; background:#007bff; color:white; border:none; border-radius:8px; font-size:18px; cursor:pointer; }
    </style>
</head>
<body>
    <div class="container">
        <h2>🩺 <%= clinic.getName() %> - 選擇預約日期</h2>
        ...
        <form action="${pageContext.request.contextPath}/timeslots.do" method="get">
            ...
            <label><strong>請選擇日期：</strong></label><br>
            <input type="date" name="appointmentDate" required min="2026-05-01" max="2026-06-30">
            
            <br><br>
            <button type="submit">查看該日期可用時間 →</button>
        </form>

        <br>
        <a href="${pageContext.request.contextPath}/patient/clinics.do">
            <button style="background-color:red">← 返回診所列表 Go back to the Clinic List</button>
            </a>
    </div>
</body>
</html>