 ...
 * @author user
 */
@WebServlet(name = "CancelAppointmentServlet", urlPatterns = {"/patient/cancel.do"})
public class CancelAppointmentServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            boolean success = adb.cancelAppointment(appointmentId, currentUser.getUserId());

            if (success) {
                request.setAttribute("message", "✅ 預約已成功取消！ Booking Cancel Success");
            } else {
                request.setAttribute("error", "❌ 無法取消此預約（可能已確認或已取消） Can't Cancel");
            }
        } catch (Exception e) {
            e.printStackTrace();
